...
-(void)refreshStateChange:(UIRefreshControl *)control
{
    ...
    if (bRet) {
        bRet = [getZegoAV_ShareInstance() getShowList:0 count:ITEM_COUNT_PER_PAGE];
...
{    
    ...
    cell.publishInfo.text = [self formatPublishInfo:dictRoom[CREATE_TIME]];
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == listData.count-1) {
        ...
        [getZegoAV_ShareInstance() getShowList:beginIndex count:ITEM_COUNT_PER_PAGE];
...
- (NSString *)formatPublishInfo:(NSString *)timeStamp{
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(nullable id)sender{
    ...
    if([segue.identifier isEqualToString:@"PlayInLiveRoom"] == YES) {
        ...
        if ([lr isKindOfClass:[LiveRoomViewController class]]) {
            lr.roomType = LIVEROOM_TYPE_PLAY;
            lr.roomNumber = dictRoom[ZEGO_ID];
            lr.roomToken = dictRoom[ZEGO_TOKEN];
            lr.createTime = dictRoom[CREATE_TIME];
