...
BEGIN_MESSAGE_MAP(CGridListCtrlEx, CListCtrl)
	...
	ON_BN_CLICKED(100000, OnItemButtonClick)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CGridListCtrlEx::OnHotTrack(NMHDR* pNMHDR, LRESULT* pResult)
{
	NMITEMACTIVATE* pItem = reinterpret_cast<NMITEMACTIVATE*>(pNMHDR);
	...
	for (int i=0; i<nColCount; i++)
	{
		...
		if (m_nLastHotTrackRow != -1 && m_nLastHotTrackRow < nRowCount)
		{
			pTrait = GetCellColumnTrait(m_nLastHotTrackRow, i);
			if (pTrait != NULL)
			{
				pTrait->OnHotTrack(*this, m_nLastHotTrackRow, i, pItem->ptAction, true);
			}
		}

		if (nRow != -1 && nRow < nRowCount)
		{
			pTrait = GetCellColumnTrait(nRow, i);
			if (pTrait != NULL)
			{
				pTrait->OnHotTrack(*this, nRow, i, pItem->ptAction, false);
			}
		}
	}

	m_nLastHotTrackRow = nRow;
	return FALSE;
}

void CGridListCtrlEx::OnMouseMove(UINT nFlags, CPoint point)
{
	if (!m_bTracking)
	{
		TRACKMOUSEEVENT tme;
		tme.cbSize = sizeof(TRACKMOUSEEVENT);
		tme.dwFlags = TME_NONCLIENT | TME_LEAVE;//要触发的消息
...
void CGridListCtrlEx::OnMouseLeave(void)
{
	// todo
	return;

	if (m_nLastHotTrackRow == -1 || m_nLastHotTrackRow >= GetItemCount())
		return;

	for (int i = 0; i < GetColumnCount(); i++)
	{
		CGridColumnTrait* pTrait = GetCellColumnTrait(m_nLastHotTrackRow, i);
		if (pTrait != NULL)
		{
			pTrait->OnHotTrack(*this, m_nLastHotTrackRow, i, CPoint(-1,-1), true);
		}
	}
...
void CGridListCtrlEx::OnItemButtonClick(void)
{
	m_signalTraitAction(m_nLastHotTrackRow);
}

sigslot::signal1<int>& CGridListCtrlEx::OnTraitSignal(void)
{
	return m_signalTraitAction;
}
